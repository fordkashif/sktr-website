...
const lookingFor = [
  {
    num: "01",
    heading: "Operators, not just founders.",
    body: "People who've built before or who understand the unsexy work of building infrastructure that lasts.",
  },
  {
    num: "02",
    heading: "Ecosystem fit.",
    body: "Products and platforms that gain something real from being inside the SKTR network — not just capital looking for a home.",
  },
  {
    num: "03",
    heading: "Long-horizon problems.",
    body: "Companies solving things that take time. We're not the right partner for a 12-month sprint to exit.",
  },
];
...
const ecosystemStack = [
  {
    label: "Capital",
    sublabel: "SKTR Ventures",
    body: "Funding structured for the long term — no artificial timeline forcing premature decisions.",
    accent: true,
  },
  {
    label: "Infrastructure",
    sublabel: "SKTR Labs",
    body: "Access to shared tooling, APIs, and R&D built specifically for performance and technology.",
    accent: false,
  },
  {
    label: "Network",
    sublabel: "SKTR Athletics",
    body: "Distribution into a real coaching and athlete ecosystem — tested at the operator level.",
    accent: false,
  },
  {
    label: "Narrative",
    sublabel: "SKTR Media",
    body: "Editorial and production reach to shape how your company is perceived inside and outside the ecosystem.",
    accent: false,
  },
];
    ...
    <>
      ...
      <section ref={heroRef} className="relative min-h-screen overflow-hidden flex flex-col">
        <motion.div className="absolute inset-0" style={{ y: bgY }}>
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-[#050608]/10 to-[#050608]/95" />
        ...
        <motion.div
          ...
          <motion.p
            className="mt-7 text-[rgba(232,235,240,0.62)] leading-relaxed max-w-[40rem]"
            style={{ fontSize: "clamp(1.05rem, 1.5vw, 1.25rem)" }}
            initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.72, ease: ease }}>
            {v.storyCopy}
          </motion.p>
          <motion.div className="mt-9 flex gap-4 flex-wrap"
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
      ...
      <main className="w-full max-w-[1700px] mx-auto px-4 sm:px-8 pb-24">
        ...
        <motion.div className="mt-20"
          ...
          <h2 className="m-0 font-extrabold leading-[0.93] tracking-[-0.06em] max-w-[18ch]"
        ...
        <div className="mt-12 flex flex-col border-t border-[rgba(131,145,190,0.15)]">
          {thesis.map((t, i) => (
            <motion.div key={t.num}
              ...
              <span className="mono text-[rgba(232,235,240,0.22)]" style={{ fontSize: "0.68rem" }}>{t.num}</span>
              <h3 className="m-0 font-extrabold leading-[1.05] tracking-[-0.04em] self-start"
                style={{ fontSize: "clamp(1.2rem, 2vw, 1.75rem)" }}>
                {t.heading}
              </h3>
              <p className="m-0 text-[rgba(232,235,240,0.52)] leading-[1.8]"
        ...
        {/* What we look for — oversized numbered list */}
        <motion.div className="mt-24"
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6, ease: ease }}>
          <p className="mono text-[rgba(232,235,240,0.38)] mb-12" style={{ fontSize: "0.68rem", letterSpacing: "0.2em" }}>
            What we look for
          </p>
          <div className="flex flex-col gap-0">
            {lookingFor.map((item, i) => (
              <motion.div key={item.num}
                className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-x-10 gap-y-3 py-10 border-t border-[rgba(131,145,190,0.12)] last:border-b group"
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.3 }}
                transition={{ duration: 0.55, delay: i * 0.08, ease: ease }}>
                {/* Large faded number */}
                <div
                  className="font-extrabold text-[rgba(62,105,255,0.12)] leading-none tracking-[-0.08em] select-none self-start group-hover:text-[rgba(62,105,255,0.22)] transition-colors duration-400"
                  style={{ fontSize: "clamp(4rem, 8vw, 7rem)", lineHeight: 1 }}>
                  {item.num}
                </div>
                <div className="flex flex-col justify-center gap-3">
                  <h3 className="m-0 font-extrabold leading-[1.05] tracking-[-0.04em]"
                    style={{ fontSize: "clamp(1.4rem, 2.8vw, 2.4rem)" }}>
                    {item.heading}
                  </h3>
                  <p className="m-0 text-[rgba(232,235,240,0.52)] leading-[1.75] max-w-[48rem]"
        ...
        {/* Beyond capital — ecosystem chain diagram */}
        <motion.div className="mt-24"
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.6, ease: ease }}>
          ...
          <h2 className="m-0 mb-12 font-extrabold leading-[0.94] tracking-[-0.06em] max-w-[22ch]"
          ...
          <div className="hidden md:grid md:grid-cols-4 gap-0">
            {ecosystemStack.map((item, i) => (
              <motion.div key={item.label}
                className={`relative flex flex-col p-6 border-t-2 ${item.accent ? "border-blue" : "border-[rgba(131,145,190,0.2)]"} bg-[rgba(8,10,16,0.4)] border-l border-r border-b border-[rgba(131,145,190,0.12)]`}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.4 }}
                transition={{ duration: 0.5, delay: i * 0.1, ease: ease }}>
                {/* Arrow connector */}
                {i < ecosystemStack.length - 1 && (
                  <div className="absolute top-[50%] -right-[0.5px] z-10 w-3 h-3 border-t border-r border-[rgba(62,105,255,0.4)] rotate-45 bg-[#050608] translate-x-1/2 -translate-y-1/2" />
                )}
                <span className="mono text-blue mb-3" style={{ fontSize: "0.6rem", letterSpacing: "0.2em" }}>
                  {item.sublabel.toUpperCase()}
                </span>
                <h3 className="m-0 mb-3 font-extrabold text-ink tracking-[-0.03em]"
                  style={{ fontSize: "clamp(1.1rem, 1.4vw, 1.35rem)" }}>
                  {item.label}
                </h3>
                <p className="m-0 text-[rgba(232,235,240,0.5)] leading-[1.7]" style={{ fontSize: "0.88rem" }}>
                  {item.body}
                </p>
              </motion.div>
            ))}
          ...
          <div className="md:hidden flex flex-col border-l-2 border-[rgba(62,105,255,0.2)] pl-6 gap-0">
            {ecosystemStack.map((item, i) => (
              <div key={item.label} className="pb-8 last:pb-0">
                <span className="mono text-blue block mb-2" style={{ fontSize: "0.6rem", letterSpacing: "0.18em" }}>
                  {item.sublabel.toUpperCase()}
                </span>
                <h3 className="m-0 mb-2 font-extrabold tracking-[-0.03em]" style={{ fontSize: "1.3rem" }}>
                  {item.label}
                </h3>
                <p className="m-0 text-[rgba(232,235,240,0.5)] leading-[1.7]" style={{ fontSize: "0.88rem" }}>
                  {item.body}
        ...
        <motion.div
          ...
          <p className="mono text-blue mb-4" style={{ fontSize: "0.68rem", letterSpacing: "0.2em" }}>Get in touch</p>
          <h3 className="m-0 mb-4 font-extrabold leading-[0.97] tracking-[-0.05em]"
