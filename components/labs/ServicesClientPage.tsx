...
function ServiceSection({ service, index }: { service: Service; index: number }) {
  return (
    <motion.div
      id={service.id}
      className="py-14 border-b border-[rgba(131,145,190,0.1)]"
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        <div className="flex items-start gap-4">
          <span className="mono text-[rgba(232,235,240,0.22)] shrink-0 mt-1" style={{ fontSize: "0.58rem" }}>
        ...
        <Link href="/contact" className="mono text-[rgba(232,235,240,0.34)] hover:text-blue transition-colors duration-150 shrink-0 sm:mt-1" style={{ fontSize: "0.66rem" }}>
          Start this project →
        </Link>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-8 lg:gap-16 mb-8">
        <p className="m-0 text-[rgba(232,235,240,0.66)] leading-[1.85]" style={{ fontSize: "clamp(0.92rem, 1.2vw, 1rem)" }}>
          {service.description}
        </p>
        <ul className="m-0 p-0 list-none flex flex-col gap-3">
          {service.included.map((item) => (
            <li key={item} className="flex items-start gap-3 text-[rgba(232,235,240,0.62)]" style={{ fontSize: "0.88rem" }}>
      ...
      <div className="flex flex-wrap items-start gap-8 pt-6 border-t border-[rgba(131,145,190,0.08)]">
        <div className="flex flex-col gap-[0.3rem]">
          <span className="mono text-[rgba(232,235,240,0.26)]" style={{ fontSize: "0.54rem", letterSpacing: "0.14em" }}>Typical timeline</span>
          <span className="font-semibold text-[rgba(232,235,240,0.78)]" style={{ fontSize: "0.9rem" }}>{service.timeline}</span>
        </div>
        <div className="w-[1px] h-8 shrink-0 self-center" style={{ background: "rgba(131,145,190,0.1)" }} />
        <div className="flex flex-col gap-[0.3rem]">
          <span className="mono text-[rgba(232,235,240,0.26)]" style={{ fontSize: "0.54rem", letterSpacing: "0.14em" }}>Best for</span>
          <span className="text-[rgba(232,235,240,0.6)]" style={{ fontSize: "0.9rem" }}>{service.bestFor}</span>
        </div>
      </div>
    </motion.div>
  );
          ...
          <motion.section className="mt-20 mb-4" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.4 }} transition={{ duration: 0.7, ease: ease }}>
            <div className="border border-[rgba(62,105,255,0.28)] px-8 sm:px-14 py-14 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
              <div>
                ...
                <p className="mt-3 m-0 text-[rgba(232,235,240,0.56)]" style={{ fontSize: "0.95rem" }}>Tell us about your project. We&apos;ll figure out the right approach together.</p>
              </div>
